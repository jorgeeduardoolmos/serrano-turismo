/* global React, Icon */
const { useState: useStateTar, useEffect: useEffectTar, useRef: useRefTar } = React;
...
const TARIFA_DATA = {
  sanpedro: {
    ...
    nights: 7, days: 8,
    base: 685000,
    includes: ["Pensión completa", "Excursiones de día completo", "Coordinadores propios", "App de seguimiento + pulsera", "Atención médica 24 h"],
    itinerary: [
  ...
  carlospaz: {
    ...
    nights: 6, days: 7,
    base: 598000,
    includes: ["Pensión completa", "Acceso a complejos & lago", "Coordinadores propios", "App de seguimiento + pulsera", "Atención médica 24 h"],
    itinerary: [
...
function AnimatedPrice({ amount }) {
  const [shown, setShown] = useStateTar(amount);
  const prev = useRefTar(amount);
  useEffectTar(() => {
    const from = prev.current, to = amount, dur = 600, t0 = performance.now();
    let raf;
    const tick = (t) => {
...
function Tarifas({ onNavigate }) {
  const [dest, setDest] = useStateTar("sanpedro");
  const [plan, setPlan] = useStateTar("6");
    ...
    <main style={{ paddingTop: "var(--nav-h)" }}>
      ...
      <section style={{ background: "var(--ink)", color: "#fff", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0,
          background: "radial-gradient(100% 120% at 85% 0%, rgba(213,98,10,.4), transparent 55%)" }} />
        <div className="wrap wrap-wide" style={{ position: "relative", paddingBlock: "clamp(50px,7vw,90px)" }}>
          ...
          <h1 style={{ color: "#fff", fontSize: "clamp(38px,5.5vw,68px)", marginTop: 16, maxWidth: "16ch" }}>
            Elegí destino, elegí cómo pagarlo
          </h1>
          <p style={{ color: "rgba(255,255,255,.82)", fontSize: 19, marginTop: 18, maxWidth: "52ch" }}>
      ...
      <section style={{ background: "var(--paper-2)", borderBottom: "1px solid var(--line)" }}>
        <div className="wrap wrap-wide" style={{ paddingBlock: 26, display: "flex",
          gap: 14, flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ fontFamily: "var(--font-head)", fontWeight: 700, fontSize: 14,
            letterSpacing: ".04em", color: "var(--ink-soft)", textTransform: "uppercase" }}>Destino</span>
          {Object.entries(TARIFA_DATA).map(([id, d]) => {
            ...
            return (
              <button key={id} onClick={() => setDest(id)} style={{
                ...
                <Icon name="pin" size={18} style={{ color: active ? "var(--accent-strong)" : "var(--muted)" }} />
                {d.name}
              </button>
            );
          })}
      ...
      <section className="section">
        <div className="wrap wrap-wide">
          <div className="price-layout" style={{ display: "grid", gridTemplateColumns: "1fr 1.05fr", gap: 48 }}>

            {/* LEFT — plan picker */}
            <div>
              <span className="eyebrow">Plan de pago</span>
              <h2 className="sec-title" style={{ fontSize: "clamp(26px,3vw,38px)" }}>¿Cómo querés pagarlo?</h2>
              <p className="sec-lead" style={{ marginTop: 14 }}>
                Elegí el plan que mejor se acomode a tu familia. El total se actualiza al instante.
              </p>

              <div className="plan-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr",
                gap: 14, marginTop: 30 }}>
                {PLANES.map((p) => {
                  const active = plan === p.id;
                  return (
                    <button key={p.id} onClick={() => setPlan(p.id)} style={{
                      position: "relative", textAlign: "left", padding: "20px 22px",
                      ...
                      {p.popular && (
                        <span style={{ position: "absolute", top: -11, right: 16,
                          background: "var(--gold)", color: "#fff", fontFamily: "var(--font-head)",
                          fontWeight: 700, fontSize: 11, letterSpacing: ".06em", padding: "4px 11px",
                          borderRadius: "var(--r-pill)" }}>POPULAR</span>
                      )}
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontFamily: "var(--font-head)", fontWeight: 800, fontSize: 19,
                          color: "var(--ink)" }}>{p.label}</span>
                        <span style={{ width: 22, height: 22, borderRadius: "50%", flex: "none",
                          border: active ? "6px solid var(--accent)" : "2px solid var(--line)",
                          background: active ? "var(--surface)" : "transparent", transition: "all .2s" }} />
                      </div>
                      <div style={{ marginTop: 6, fontSize: 14, color: "var(--ink-soft)" }}>{p.note}</div>
                      {p.badge && <span className="chip gold" style={{ marginTop: 12 }}>{p.badge}</span>}
                    </button>
                  );
                })}
              </div>

              <div style={{ marginTop: 26, padding: "18px 22px", borderRadius: "var(--r-md)",
                background: "var(--paper-2)", display: "flex", gap: 12, alignItems: "center" }}>
                <Icon name="shield" size={22} style={{ color: "var(--accent-strong)", flex: "none" }} />
            ...
            <div style={{ position: "sticky", top: "calc(var(--nav-h) + 24px)", alignSelf: "start" }}>
              <div style={{ position: "relative", borderRadius: "var(--r-xl)", overflow: "hidden",
                background: "var(--ink)", color: "#fff", boxShadow: "var(--sh-lg)" }}>
                <div style={{ position: "absolute", inset: 0,
                  background: "radial-gradient(110% 90% at 110% -10%, rgba(213,98,10,.55), transparent 60%)" }} />
                <div style={{ position: "relative", padding: "clamp(28px,3vw,42px)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      <div style={{ color: "var(--gold-soft)", fontFamily: "var(--font-head)",
                        fontWeight: 600, fontSize: 13, letterSpacing: ".16em", textTransform: "uppercase" }}>
                        {data.region}
                      </div>
                      <h3 style={{ color: "#fff", fontSize: 30, marginTop: 6 }}>{data.name}</h3>
                    </div>
                    <span className="chip glass" style={{ background: "rgba(255,255,255,.14)" }}>
                      <Icon name="calendar" size={14} /> {data.days} días · {data.nights} noches
                    </span>
                  </div>

                  <div style={{ marginTop: 30, paddingTop: 26, borderTop: "1px solid rgba(255,255,255,.14)" }}>
                    <div style={{ fontSize: 14.5, color: "rgba(255,255,255,.7)" }}>
                      {plan === "contado" ? "Precio final por pasajero" : `${cuotas} cuotas de`}
                    </div>
                    <div className="kicker-num" style={{ fontSize: "clamp(46px,6vw,68px)", color: "#fff",
                    ...
                    {plan === "contado" && (
                      <div style={{ marginTop: 8, fontSize: 15, color: "var(--gold-soft)", fontWeight: 600 }}>
                  ...
                  <div style={{ marginTop: 26, display: "grid", gap: 11 }}>
                    {data.includes.map((inc) => (
                      <div key={inc} style={{ display: "flex", alignItems: "center", gap: 11, fontSize: 15 }}>
                        <span style={{ width: 22, height: 22, borderRadius: "50%", flex: "none",
                          background: "rgba(196,154,61,.22)", display: "grid", placeItems: "center",
                          color: "var(--gold-soft)" }}>
                  ...
                  <button className="btn btn-primary btn-lg" style={{ width: "100%", marginTop: 30 }}
                    onClick={() => onNavigate("contacto")}>
                    Reservar este plan <Icon name="arrow" size={19} />
                  </button>
                  <p style={{ textAlign: "center", marginTop: 14, fontSize: 13, color: "rgba(255,255,255,.55)" }}>
      ...
      <section className="section dots" style={{ background: "var(--paper-2)" }}>
        <div className="wrap wrap-wide">
          ...
          <div className="itin-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)",
            gap: 20, marginTop: 46 }}>
            {data.itinerary.map(([days, title, text], i) => (
              <div key={i} style={{ background: "var(--surface)", borderRadius: "var(--r-lg)",
                overflow: "hidden", border: "1px solid var(--line-soft)", boxShadow: "var(--sh-sm)",
                display: "flex", flexDirection: "column" }}>
                <div style={{ height: 110, position: "relative",
                  background: "linear-gradient(135deg, var(--accent), var(--brand-strong))" }}>
                  <span style={{ position: "absolute", top: 16, left: 18, fontFamily: "var(--font-head)",
                    fontWeight: 800, fontSize: 56, color: "rgba(255,255,255,.28)", lineHeight: 1 }}>
                    0{i + 1}
                  </span>
                  <span style={{ position: "absolute", bottom: 14, left: 18, color: "#fff",
                    fontFamily: "var(--font-head)", fontWeight: 700, fontSize: 14, letterSpacing: ".04em" }}>
                    {days}
                  </span>
                </div>
                <div style={{ padding: "22px 22px 26px" }}>
                  <h3 style={{ fontSize: 19 }}>{title}</h3>
                  <p style={{ marginTop: 10, fontSize: 14.5, color: "var(--ink-soft)", lineHeight: 1.55 }}>{text}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 40, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center",
            ...
            <button className="btn btn-dark btn-lg" onClick={() => onNavigate("destino")}>
              Ver todo el destino <Icon name="arrow" size={19} />
            </button>
            <button className="btn btn-outline btn-lg" onClick={() => onNavigate("contacto")}>
